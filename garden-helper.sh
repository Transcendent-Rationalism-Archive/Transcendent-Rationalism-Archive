#!/bin/bash

echo "🌿 Помощник по работе с Садом Сознаний"

# 1. Пулл
read -p "1. Выполнить git pull (синхронизировать с GitHub)? [y/N]: " -n 1 -r
echo
if [[ $REPLY =~ ^[Yy]$ ]]; then
    git pull origin main
fi

# 2. Выбор файлов для добавления
echo "2. Выберите, какие изменения добавить в коммит:"
echo "   a) Все изменения (git add .)"
echo "   b) Конкретный файл"
echo "   c) Пропустить этот шаг"
read -p "Ваш выбор [a/b/c]: " choice

case $choice in
    a|A) git add . ;;
    b|B) 
        read -p "Введите путь к файлу: " filepath
        git add "$filepath"
        ;;
    *) echo "Изменения не добавлены." ;;
esac

# 3. Коммит
read -p "3. Введите сообщение коммита: " commit_msg
if [ -n "$commit_msg" ]; then
    git commit -m "$commit_msg"
else
    echo "Сообщение коммита пустое. Коммит не создан."
    exit 1
fi

# 4. Пуш
read -p "4. Выполнить git push (отправить изменения)? [y/N]: " -n 1 -r
# 3. Коммит
# Если сообщение передано как аргумент скрипта (./garden-helper.sh "сообщение"), используем его.
if [ -n "$1" ]; then
    commit_msg="$1"
    echo "3. Использую переданное сообщение коммита: \"$commit_msg\""
else
    # Иначе запрашиваем у пользователя
    read -p "3. Введите сообщение коммита: " commit_msg
fi

if [ -n "$commit_msg" ]; then
    git commit -m "$commit_msg"
else
    echo "Сообщение коммита пустое. Коммит не создан."
    exit 1
fi
if [[ $REPLY =~ ^[Yy]$ ]]; then
    git push origin main
fi

echo "✅ Работа помощника завершена."
